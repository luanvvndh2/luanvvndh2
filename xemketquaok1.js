document.addEventListener("DOMContentLoaded", function () {
    ...
    searchButton.addEventListener("click", async function (event) {
        ...
        if (maTheBhyt || soCccd) {
            ...
            resultTable.style.display = "none";  // Ẩn bảng kết quả khi đang tìm kiếm
            resultTableBody.innerHTML = "";      // Xóa dữ liệu cũ
    ...
    async function handleSearch(maTheBhyt, soCccd) {
        try {
            ...
            const ketquaResponse = await fetch(`${API_URL}/ketqua?MA_LK.eq.${maLk}`, {
                headers: {
                    "Content-Type": "application/json",
                    "apikey": API_KEY,
                    "Authorization": `Bearer ${API_KEY}`
            ...
            // Loại bỏ kết quả trùng lặp dựa trên MA_DICH_VU và STT
            const uniqueResults = Array.from(
                new Map(
                    ketquaData.map(item => [`${item.MA_DICH_VU}-${item.STT}`, item])
                ).values()
            );
            ...
            uniqueResults.forEach(item => {
                ...
                row.innerHTML = `
                    ...
                    <td>${item.GIA_TRI}</td>
                <td>${item.KET_LUAN || "Không có kết luận"}</td>
                `;
            ...
            searchMessage.textContent = "Có lỗi xảy ra khi tìm kiếm. Vui lòng thử lại sau!";
        }
    }
});
