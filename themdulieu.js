document.addEventListener("DOMContentLoaded", function () {
    ...
    async function handleImportThongTin() {
        ...
        //thuốc
         async function handleImportthuoc() {
        const fileInput = document.getElementById("filethuoc");
        const message = document.getElementById("messagethuoc");

        if (!fileInput.files.length) {
            message.textContent = "Vui lòng chọn file Excel cho bảng thuốc bảng 2 !";
            return;
        }
        ...
        // Kiểm tra và loại bỏ dữ liệu trùng lặp
        const filteredData = await filterDuplicateData(data, "thuoc");
        if (filteredData.length === 0) {
            message.textContent = "Tất cả dữ liệu đều đã tồn tại trong hệ thống.";
            return;
        }
        ...
        message.textContent = "Đang tải dữ liệu lên Supabase...";
        const response = await uploadToSupabase(filteredData, "thuoc");
        message.textContent = response.ok ? "Import thuốc   thành công!" : "Có lỗi xảy ra khi import thuốc.";
    }  
...
https://cdn.jsdelivr.net/gh/luanvvndh2/luanvvndh2/0.15.6xlsx.full.min.js
