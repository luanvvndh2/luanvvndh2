    ...
    searchButton.addEventListener("click", async (event) => {
        ...
        try {
            ...
            if (data && Array.isArray(data) && data.length > 0) {
                searchMessage.textContent = `Tìm thấy ${data.length} kết quả dịch vụ.`;
                ...
                for (const [ngay, records] of Object.entries(groupedData)) {
                    ...
                    ngayRow.innerHTML = `<td colspan="5" style="background-color:#f0f0f0; font-weight:bold; text-align:center;">Ngày khám: ${ngay}</td>`;
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const hotenTInput = document.getElementById("hotenTInput");
    const namsinhTInput = document.getElementById("namsinhTInput");
    const searchMessage1 = document.getElementById("searchMessage1");
    const resultTableBody1= document.querySelector("#resultTable1 tbody");
    ...
    searchButton.addEventListener("click", async (event) => {
        ...
        resultTableBody1.innerHTML = "";
        searchMessage1.textContent = "Đang tìm kiếm thuốc...";

     
        try {
            const response1 = await fetch(`${supabaseUrl}/rest/v1/rpc/tim_kiem_thuoc`, {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                    apikey: supabaseKey,
            ...
            if (data && Array.isArray(data) && data.length > 0) {
                searchMessage.textContent = `Tìm thấy ${data.length} kết quả thuốc`;

                const groupedData = {};
                data.forEach(item => {
                    const ngayXetNghiem = item.ngay_yl || "Không rõ ngày ";
                    if (!groupedData[ngayXetNghiem]) {
                        groupedData[ngayXetNghiem] = [];
                    }
                    groupedData[ngayXetNghiem].push(item);
                ...
                for (const [ngay, records] of Object.entries(groupedData)) {
                    const ngayRow = document.createElement("tr");
                    ngayRow.innerHTML = `<td colspan="5" style="background-color:#f0f0f0; font-weight:bold; text-align:center;">Ngày khám: ${ngay}</td>`;
                    resultTableBody1.appendChild(ngayRow);
                    ...
                    records.forEach(record => {
                        ...
                        `;
                        resultTableBody1.appendChild(row);
                    });
                }
            } else {
               // searchMessage.textContent = "Không tìm thấy kết quả phù hợp.";
